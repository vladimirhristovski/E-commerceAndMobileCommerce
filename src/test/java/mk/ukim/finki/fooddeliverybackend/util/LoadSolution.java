...
public class LoadSolution {
    ...
    private static Integer examId = 4;
...
//        commitSolution(index);
    ...
    public static void getAndLoadLastSolution(String index) throws IOException {
        ...
        ResponseEntity<String> response = restTemplate.exchange("http://localhost:8080/api/grading/" + examId + "/last_submission/full/" + index, HttpMethod.GET, new HttpEntity<>(createHeaders(System.getenv("WP_USER"), System.getenv("WP_PASS"))), String.class);
    ...
    public static void getSolutionById(Integer id) throws IOException {
        RestTemplate restTemplate = new RestTemplate();
        ResponseEntity<String> response = restTemplate.exchange("http://localhost:8080/api/grading/submission/" + id, HttpMethod.GET, new HttpEntity<>(createHeaders(System.getenv("WP_USER"), System.getenv("WP_PASS"))), String.class);
        loadSolution(new ByteArrayInputStream(response.getBody().getBytes()));
    }

    public static void loadSolution(InputStream response) throws IOException {
        HashMap<String, String> solution = objectMapper.reader().forType(new TypeReference<HashMap<String, String>>() {
        }).readValue(response);
        if (solution.isEmpty()) {
        ...
        solution.forEach((key, value) -> {
            String normalizedPath = key.replace("\\", "/");
            ...
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(file, false))) {
                ...
                System.err.println("Failed to write: " + file.getPath());
                e.printStackTrace();
            }
        });
    }
